...
for idx, group in enumerate(filtered_groups):
    ...
    #图2 Pw vs dc2
    plt.figure(figsize=(6, 5))
    plt.scatter(dc2, Pw, c=L, cmap='turbo', s=8, alpha=0.6)
    #plt.plot(dc2, Pw, '-o', markersize=4, alpha=0.6, color='blue')  # 使用折线图带点标记
    plt.xlabel("dc2 (mm)")
    plt.ylabel("绕组损耗 Pw (W)")
    plt.title(f"组 {idx + 1} | 绕组损耗 vs dc2")
    plt.colorbar(label="L(uH)")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    # 图3 Pc vs dc2
    plt.figure(figsize=(6, 5))
    plt.scatter(dc2, Pc, c=L, cmap='turbo', s=8, alpha=0.6)
    #plt.plot(dc2, Pc, '-o', markersize=4, alpha=0.6, color='blue')  # 使用折线图带点标记
    plt.xlabel("dc2 (mm)")
    plt.ylabel("磁芯损耗 Pc (W)")
    plt.title(f"组 {idx + 1} | 磁芯损耗 vs dc2")
    plt.colorbar(label="L(uH)")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    # 图4 P vs dc2
    plt.figure(figsize=(6, 5))
    plt.scatter(dc2, P, c=L, cmap='turbo', s=8, alpha=0.6)
    # plt.plot(dc2, Pc, '-o', markersize=4, alpha=0.6, color='blue')  # 使用折线图带点标记
    plt.xlabel("dc2 (mm)")
    plt.ylabel("总损耗 P (W)")
    plt.title(f"组 {idx + 1} | 总损耗 vs dc2")
    plt.colorbar(label="L(uH)")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
